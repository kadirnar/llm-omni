...
class SiglipVisionEmbeddings(nn.Module):
    def __init__(self, config: SiglipVisionConfig):
        ...
        slef.patch_size = config.patch_size
        ...
        self.position_embedding = nn.Embedding(self.num_image_tokens, self.embed_dim)
        self.register_buffer(
            "position_ids", 
            torch.arange(self.num_image_tokens).expand((1, -1)),
            persistent=False,
        )
...
class SiglipAttention(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor) -> Tuple[torch.Tensor, Optional[torch.Tensor]]:
        ...
        attn_weights = self.attn_dropout(attn_weights, p=self.dropout, training=self.training)
...
class SiglipEncoderLayer(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        # [Batch_Size, Num_Patches, Embed_Dim]
        hidden_states = residual1 + hidden_states
        # [Batch_Size, Num_Patches, Embed_Dim]
